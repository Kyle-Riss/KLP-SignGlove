...
def train_s_gru_full_data(model, all_data, all_labels, device, epochs=100, learning_rate=0.001):
    """전체 데이터로 S-GRU 모델 훈련"""
...
def save_s_gru_model(model, filepath='s_gru_full_model.pth'):
    """S-GRU 모델 저장"""
...
def create_training_visualization(history, class_names):
    ...
    fig.suptitle('🚀 S-GRU 전체 데이터 훈련 과정', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('s_gru_full_training_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print('✅ 훈련 과정 시각화 저장: s_gru_full_training_analysis.png')

def main():
    """메인 실행 함수"""
    print('🚀 S-GRU 전체 데이터 훈련 시작')
    print('=' * 70)
    ...
    # 모델 훈련 (전체 데이터 사용)
    history = train_s_gru_full_data(
        s_gru_model, all_data_processed, all_labels, device, epochs=100, 
        learning_rate=0.001
    )
    
    # 모델 저장
    save_s_gru_model(s_gru_model, 'models/s_gru_full_model.pth')
    
    # 결과 요약
    print('\n📊 S-GRU 전체 데이터 훈련 결과 요약')
    print('=' * 70)
    ...
    print(f'모델 저장: models/s_gru_full_model.pth')
