...
class TestAntiOverfittingDataset(Dataset):
    """과적합 방지 모델 테스트용 데이터셋"""
    ...
    def __init__(self, data_path, sequence_length=20, validation_type='user_split', random_seed=42):
        ...
        # 데이터 로드
        self.data, self.labels, self.class_indices = self.load_test_data(validation_type)
        
        # 데이터 정규화
        self.normalize_data()
        
    def load_test_data(self, validation_type):
        """테스트 데이터 로드"""
        print(f"📊 {validation_type} 테스트 데이터 로딩 중...")
...
class AntiOverfittingTester:
    ...
    def __init__(self, model_path='best_anti_overfitting_model.pth', device='cuda' if torch.cuda.is_available() else 'cpu'):
        ...
        print(f"✅ 과적합 방지 모델 로드 완료 (디바이스: {device})")
    ...
    def analyze_results(self, predictions, labels, probabilities, validation_type):
        """결과 분석"""
        print(f"\n📊 {validation_type} 과적합 방지 모델 결과 분석")
        print("=" * 60)
...
def main():
    ...
    tester = AntiOverfittingTester()
    
    for validation_type in validation_types:
        print(f"\n🔍 {validation_type} 테스트 시작")
        print("-" * 40)
        
        # 테스트 데이터 로드
        test_dataset = TestAntiOverfittingDataset(
        ...
        # 성능 테스트
        predictions, labels, probabilities = tester.test_performance(test_loader)
        
        if predictions is not None:
            # 결과 분석
            result = tester.analyze_results(predictions, labels, probabilities, validation_type)
            ...
            print(f"❌ {validation_type} 테스트를 완료할 수 없습니다.")
    ...
    # 결과 저장
    with open('anti_overfitting_test_results.json', 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    
    print(f"\n📁 결과가 'anti_overfitting_test_results.json'에 저장되었습니다.")
