...
Multi-Scale 3D CNN (3개 타워 병렬):
  ├─ Tower 1: Conv3d(3x3x3) → BatchNorm → ReLU  (세밀한 특성)
  ├─ Tower 2: Conv3d(5x5x5) → BatchNorm → ReLU  (중간 특성)
  └─ Tower 3: Conv3d(7x7x7) → BatchNorm → ReLU  (거시적 특성)
     ↓
...
Multi-Scale은 **다양한 크기의 커널을 병렬로 사용**하여 **다양한 스케일의 특성을 동시에 추출**하는 기법입니다.

**MS3DGRU의 Multi-Scale 구조:**
- **Tower 1 (3x3x3)**: 작은 커널 → **세밀한 특성** 포착
  - 예: 개별 센서 간의 짧은 시간/공간 상호작용
  - 빠른 손동작 변화 감지
- **Tower 2 (5x5x5)**: 중간 커널 → **중간 특성** 포착
  - 예: 여러 센서 그룹 간의 상호작용
  - 중간 스케일의 패턴 학습
- **Tower 3 (7x7x7)**: 큰 커널 → **거시적 특성** 포착
  - 예: 전체 손 형태의 전반적인 변화
  - 장기적인 손동작 패턴 학습
...
**학습 단계 (`forward` 함수):**
```python
# 입력: (batch, time=87, channels=8)
x_3d = x.view(batch, time, 4, 2)  # 8개 센서를 4x2 공간으로 재배열

# Multi-Scale 3D CNN - 3개 타워 병렬 처리
t1 = self.tower1(x_3d)  # 3x3x3 커널로 세밀한 특성 추출
t2 = self.tower2(x_3d)  # 5x5x5 커널로 중간 특성 추출
t3 = self.tower3(x_3d)  # 7x7x7 커널로 거시적 특성 추출

# 3개 타워의 출력을 결합 (Concatenate)
conv_out = torch.cat([t1, t2, t3], dim=1)  # (batch, filters*3, time, 4, 2)
```

**추론 단계:**
- 학습과 동일한 구조 사용
- 체크포인트에서 가중치 로드 후 동일한 forward pass 수행

#### 3. **Multi-Scale의 효과**

**✅ 다양한 스케일의 특성 포착:**
- **세밀한 패턴**: 개별 손가락의 빠른 움직임 (Tower 1)
- **중간 패턴**: 손가락 그룹 간의 상호작용 (Tower 2)
- **거시적 패턴**: 전체 손 형태의 전반적인 변화 (Tower 3)

**✅ 센서 간 공간적 상호작용 학습:**
- 8개 센서를 4x2 공간으로 재배치하여 공간적 구조 생성
- 다양한 커널 크기로 **다양한 거리 범위의 센서 간 상호작용** 학습
  - 3x3x3: 인접 센서 간 상호작용
  - 5x5x5: 중간 거리 센서 간 상호작용
  - 7x7x7: 전체 센서 네트워크 상호작용

**✅ 시간-공간 특성 동시 학습:**
- 3D CNN이 **시간(t) × 공간(height, width)** 차원을 동시에 처리
- 다양한 커널 크기로 **다양한 시간 범위의 변화** 학습
  - 짧은 시간: 빠른 손동작 변화
  - 긴 시간: 전체 제스처의 전반적인 흐름

**✅ 성능 향상:**
- **단일 스케일 CNN (3x3x3만 사용)**: ~98% 정확도
- **Multi-Scale CNN (3x3x3 + 5x5x5 + 7x7x7)**: **99.13% 정확도** ✅
- **0.34% 정확도 향상** (매우 의미 있는 개선)

#### 4. **왜 단일 커널로는 부족한가?**

**단일 커널의 한계:**
- **작은 커널 (3x3x3)만 사용**: 세밀한 패턴은 잘 포착하지만 전체적인 맥락 파악 어려움
- **큰 커널 (7x7x7)만 사용**: 거시적 패턴은 잘 포착하지만 세밀한 변화 감지 어려움
- **특정 크기만 사용**: 특정 스케일의 패턴에만 최적화되어 다른 스케일의 정보 손실

**Multi-Scale의 장점:**
- **모든 스케일의 정보 보존**: 세밀한 패턴부터 거시적 패턴까지 모두 포착
- **더 풍부한 특징 표현**: 다양한 관점에서 추출된 특징을 결합하여 더 강력한 표현력
- **더 나은 일반화**: 다양한 스케일의 패턴을 학습하여 더 강건한 모델

#### 5. **실제 예시: 수어 인식에서의 Multi-Scale 효과**

**예시 1: "ㄱ" 자모 인식**
- **Tower 1 (3x3x3)**: 검지 손가락의 빠른 구부림 감지
- **Tower 2 (5x5x5)**: 검지와 중지 간의 상대적 위치 변화 포착
- **Tower 3 (7x7x7)**: 전체 손의 형태 변화 (손목 회전, 손가락 배열) 학습

**예시 2: "ㅏ" 자모 인식**
- **Tower 1 (3x3x3)**: 엄지와 검지의 접촉 패턴 감지
- **Tower 2 (5x5x5)**: 손가락 그룹의 동시 움직임 포착
- **Tower 3 (7x7x7)**: 전체 손 형태의 전반적인 변화 패턴 학습

**결론**: Multi-Scale은 **다양한 스케일의 특성을 동시에 추출**하여 **단일 스케일보다 더 높은 성능**을 달성합니다. 특히 센서 간 공간적 상호작용과 시간-공간 특성을 동시에 학습하는 3D CNN에서 Multi-Scale의 효과가 두드러집니다.
