#!/usr/bin/env python3
"""
균형잡힌 Episode 데이터 학습 스크립트
과적합 방지를 위한 보수적인 학습 전략
"""
...
class BalancedEpisodeDataset(Dataset):
    """과적합 방지를 위한 균형잡힌 Episode 데이터셋"""
    
    def __init__(self, data_dir, window_size=20, stride=5, augment=True, target_samples_per_class=500):
    ...
    def _load_episode_data(self):
        ...
        for class_name, windows in class_data.items():
            ...
            if self.augment and len(windows) < self.target_samples_per_class:
                augmented_windows = self._augment_data_conservative(windows, self.target_samples_per_class)
    ...
    def _augment_data_conservative(self, windows, target_count):
        """보수적인 데이터 증강"""
        if len(windows) == 0:
            return []
        ...
        while len(augmented) < target_count:
            ...
            # 보수적인 증강 기법 적용
            augmented_window = self._apply_conservative_augmentation(original)
            augmented.append(augmented_window)
        
        return augmented[:target_count]
    
    def _apply_conservative_augmentation(self, window):
        """보수적인 증강 적용"""
        # 매우 작은 노이즈만 추가
...
class BalancedEpisodeTrainer:
    ...
    def create_dataset(self) -> BalancedEpisodeDataset:
        """균형잡힌 데이터셋 생성"""
        dataset = BalancedEpisodeDataset(
    ...
    def train_model(self, dataset: BalancedEpisodeDataset):
        """모델 학습"""
        print("🚀 균형잡힌 Episode 데이터로 모델 학습 시작...")
        ...
        model = DeepLearningPipeline(
            ...
            hidden_dim=128,  # 더 작은 hidden dimension
            num_layers=2,    # 더 얕은 네트워크
        ...
        for epoch in range(self.config['epochs']):
            ...
            if epoch > 5:
                ...
                if train_val_gap > 5.0:  # 훈련-검증 정확도 차이가 5% 이상
                    print(f"  ⚠️ 과적합 의심: 훈련-검증 차이 = {train_val_gap:.2f}%")
            ...
            if avg_val_loss < best_val_loss:
                ...
                if self.config['save_best_only']:
                    torch.save(model, 'best_balanced_episode_model.pth')
        ...
        # 최종 모델 저장
        torch.save(model, 'final_balanced_episode_model.pth')
    ...
    def _plot_training_curves(self, train_losses, val_losses, train_accuracies, val_accuracies):
        ...
        plt.tight_layout()
        plt.savefig('balanced_episode_training_curves.png', dpi=300, bbox_inches='tight')
        print("✅ 학습 곡선 저장: balanced_episode_training_curves.png")
        plt.show()
    ...
    def _plot_confusion_matrix(self, targets, predictions):
        ...
        plt.tight_layout()
        plt.savefig('balanced_episode_confusion_matrix.png', dpi=300, bbox_inches='tight')
        print("✅ 혼동 행렬 저장: balanced_episode_confusion_matrix.png")
        plt.show()
...
def main():
    """메인 함수"""
    print("🚀 균형잡힌 Episode 데이터로 모델 재학습 시작")
    print("=" * 60)
    
    # 학습기 초기화
    trainer = BalancedEpisodeTrainer()
    ...
    print(f"✅ 최고 모델: best_balanced_episode_model.pth")
    print(f"✅ 최종 모델: final_balanced_episode_model.pth")
