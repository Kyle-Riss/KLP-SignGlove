...
class ImprovedEnsembleModel(nn.Module):
    ...
    def __init__(self, num_classes=24):
        ...
        # 동적 앙상블 가중치 학습
        self.confusion_weight = nn.Parameter(torch.tensor(0.8))  # 혼동 클래스용 특화 모델 가중치
        self.general_weight = nn.Parameter(torch.tensor(0.2))    # 일반 클래스용 일반 모델 가중치
    ...
    def forward(self, x, is_confusion=None):
        ...
        if is_confusion is not None:
            # 가중치 정규화 (0~1 범위)
            confusion_weight = torch.sigmoid(self.confusion_weight)
            general_weight = torch.sigmoid(self.general_weight)
            
            # 혼동 클래스는 특화 모델에 높은 가중치, 일반 클래스는 일반 모델에 높은 가중치
...
class ImprovedEnsembleTrainer:
    ...
    def __init__(self, config):
        ...
        print(f"🚀 개선된 앙상블 모델 훈련 시작")
        print(f"📱 디바이스: {self.device}")
        print(f"🎯 동적 가중치 조정으로 혼동 클래스 성능 향상")
    
    def create_model(self):
        """앙상블 모델 생성"""
        model = ImprovedEnsembleModel(num_classes=24).to(self.device)
    ...
    def create_optimizer(self, model):
        """옵티마이저 생성"""
        # 앙상블 가중치와 모델 파라미터 모두 학습
        optimizer = optim.AdamW([
            {'params': [model.confusion_weight, model.general_weight], 'lr': self.config['learning_rate'] * 2.0},
    ...
    def train(self, train_loader, val_loader, model, optimizer, scheduler, criterion):
        ...
        for epoch in range(self.config['epochs']):
            ...
            # 앙상블 가중치 출력
            confusion_weight = outputs['confusion_weight'].item()
            general_weight = outputs['general_weight'].item()
            
            print(f"Epoch {epoch+1}/{self.config['epochs']}:")
            print(f"  Train Loss: {avg_train_loss:.4f}, Train Acc: {train_acc:.2f}%")
            print(f"  Val Loss: {avg_val_loss:.4f}, Val Acc: {val_acc:.2f}%")
            print(f"  Weights: Confusion={confusion_weight:.3f}, General={general_weight:.3f}")
            ...
            if avg_val_loss < best_val_loss:
                ...
                patience_counter = 0
                torch.save(model.state_dict(), 'best_improved_ensemble_model.pth')
    ...
    def _plot_training_curves(self, train_losses, val_losses):
        ...
        plt.title('Improved Ensemble Model Training and Validation Loss')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend()
        ...
        plt.tight_layout()
        plt.savefig('improved_ensemble_training_curves.png', dpi=300, bbox_inches='tight')
        plt.close()
        print("✅ 훈련 곡선 저장: improved_ensemble_training_curves.png")
    ...
    def _create_confusion_matrix(self, targets, predictions):
        ...
        with open('improved_ensemble_classification_report.json', 'w', encoding='utf-8') as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        
        print("✅ 혼동 행렬 저장: improved_ensemble_confusion_matrix.png")
        print("✅ 분류 보고서 저장: improved_ensemble_classification_report.json")
...
def main():
    ...
    # 모델 생성
    trainer = ImprovedEnsembleTrainer(config)
