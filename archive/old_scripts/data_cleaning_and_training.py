...
class DataCleaner:
    ...
    def clean_data(self, data_dir):
                            ...
                            if file_name.endswith('.h5'):
                                ...
                                try:
                                    with h5py.File(file_path, 'r') as f:
                                        ...
                                        # 데이터 정제
                                        cleaned_data = self._clean_single_sample(sensor_data)
        ...
        print(f'✅ 1순위 정제 완료: {self.removed_samples}개 샘플 제거, {self.normalized_samples}개 샘플 정규화')
        return self.cleaned_data
    
    def _clean_single_sample(self, sensor_data):
        """단일 샘플 정제"""
        # 1. 범위 오류 검사 및 제거
        ...
        for i, sensor_name in enumerate(sensor_names):
            ...
            if i < 5 and std_val > 200:
                ...
                # 적절한 범위로 스케일링 (표준편차 50으로)
                cleaned_data[:, i] = mean_val + normalized * 50
                self.normalized_samples += 1
        
        return cleaned_data

class EnhancedH5Dataset(torch.utils.data.Dataset):
    """2순위 정제: 낮은 변동성 증강, 클래스별 특화 전처리"""
    ...
    def _class_specific_augmentation(self, data, class_name):
        ...
        # 문제 클래스별 특화 증강
        if class_name in ['ㅕ', 'ㅠ', 'ㅡ']:  # Flex2 변동성 과다 클래스
            # Flex2 센서 안정화
        ...
        elif class_name in ['ㄱ', 'ㄴ', 'ㄹ', 'ㅁ', 'ㅂ', 'ㅅ', 'ㅇ', 'ㅊ', 'ㅓ']:  # 낮은 변동성 클래스
            # Orientation 센서 증강
            for noise_level in [0.05, 0.1, 0.15]:
                aug_data = data.copy()
                # Orientation 센서에만 노이즈 추가
