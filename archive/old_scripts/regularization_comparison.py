...
class GRUModelL2(nn.Module):
    """L2 규제가 적용된 GRU 모델"""
    
    def __init__(self, input_size=8, hidden_size=64, num_layers=2, num_classes=24, dropout=0.6):
        super(GRUModelL2, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
...
class GRUModelL1(nn.Module):
    """L1 규제가 적용된 GRU 모델"""
    
    def __init__(self, input_size=8, hidden_size=64, num_layers=2, num_classes=24, dropout=0.6):
        super(GRUModelL1, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
...
class GRUModelElasticNet(nn.Module):
    """Elastic Net 규제가 적용된 GRU 모델 (L1 + L2)"""
    
    def __init__(self, input_size=8, hidden_size=64, num_layers=2, num_classes=24, dropout=0.6):
        super(GRUModelElasticNet, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
...
def train_model_with_regularization(model, train_loader, val_loader, model_name, 
                                  l1_weight=0.0, l2_weight=0.0, epochs=100, device='cuda'):
    """규제가 적용된 모델 훈련"""
    model.to(device)
    criterion = nn.CrossEntropyLoss()
    
    # 규제 유형에 따른 optimizer 설정
    if l1_weight > 0 and l2_weight > 0:
        # Elastic Net
        optimizer = torch.optim.AdamW(model.parameters(), lr=0.001, weight_decay=l2_weight)
        print(f'📊 {model_name}: Elastic Net (L1={l1_weight}, L2={l2_weight})')
    elif l1_weight > 0:
        # L1 규제
        optimizer = torch.optim.AdamW(model.parameters(), lr=0.001, weight_decay=0)
        print(f'📊 {model_name}: L1 규제 (weight={l1_weight})')
    elif l2_weight > 0:
        # L2 규제
        optimizer = torch.optim.AdamW(model.parameters(), lr=0.001, weight_decay=l2_weight)
        print(f'📊 {model_name}: L2 규제 (weight={l2_weight})')
    else:
        # 규제 없음
        optimizer = torch.optim.AdamW(model.parameters(), lr=0.001, weight_decay=0)
        print(f'📊 {model_name}: 규제 없음')
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in train_loader:
            ...
            # L1 규제 추가 (L2는 optimizer의 weight_decay로 처리)
            if l1_weight > 0:
                l1_norm = sum(torch.norm(p, 1) for p in model.parameters())
                loss += l1_weight * l1_norm
...
if __name__ == "__main__":
    ...
    regularization_configs = {
        'NoReg': {'l1_weight': 0.0, 'l2_weight': 0.0},
        'L2': {'l1_weight': 0.0, 'l2_weight': 1e-4},
        'L1': {'l1_weight': 1e-4, 'l2_weight': 0.0},
        'ElasticNet': {'l1_weight': 5e-5, 'l2_weight': 5e-5}
    }
    
    # 모델 정의
    models = {
        'NoReg': GRUModelNoReg(input_size=8, hidden_size=64, num_layers=2, num_classes=24, dropout=0.6),
        'L2': GRUModelL2(input_size=8, hidden_size=64, num_layers=2, num_classes=24, dropout=0.6),
        'L1': GRUModelL1(input_size=8, hidden_size=64, num_layers=2, num_classes=24, dropout=0.6),
        'ElasticNet': GRUModelElasticNet(input_size=8, hidden_size=64, num_layers=2, num_classes=24, dropout=0.6)
    }
    ...
    for model_name, model in models.items():
        ...
        config = regularization_configs[model_name]
        
        # 훈련
        train_losses, val_losses, train_accuracies, val_accuracies, best_val_acc = train_model_with_regularization(
            model, train_loader, val_loader, model_name,
            l1_weight=config['l1_weight'], l2_weight=config['l2_weight'],
    ...
    # 성능 비교 테이블
    print(f'{"모델":<12} {"L1 가중치":<10} {"L2 가중치":<10} {"검증 정확도":<12} {"테스트 정확도":<12}')
    print(f'{"-"*70}')
    
    for model_name, result in results.items():
        config = result['config']
        print(f'{model_name:<12} {config["l1_weight"]:<10.0e} {config["l2_weight"]:<10.0e} '
              f'{result["best_val_acc"]:<12.2f} {result["test_accuracy"]:<12.2f}')
    ...
    fig.suptitle('규제 기법별 훈련 과정 비교', fontsize=16, fontweight='bold')
    
    colors = {'NoReg': 'blue', 'L2': 'red', 'L1': 'green', 'ElasticNet': 'orange'}
    
    for i, model_name in enumerate(['NoReg', 'L2', 'L1', 'ElasticNet']):
    ...
    plt.tight_layout()
    plt.savefig('regularization_performance.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f'\n🎉 규제 기법 비교 완료!')
    print(f'💾 저장된 파일:')
    print(f'  - noreg_model.pth, l2_model.pth, l1_model.pth, elasticnet_model.pth: 각 모델')
    print(f'  - regularization_comparison.png: 훈련 과정 비교')
    print(f'  - regularization_performance.png: 성능 비교')
