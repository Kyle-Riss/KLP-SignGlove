...
class DataCleaner:
    ...
    def clean_data(self, data_dir):
        ...
        print(f'✅ 1순위 정제 완료: {sum(self.removed_samples.values())}개 제거, {sum(self.normalized_samples.values())}개 정규화')
        ...
        for class_name in sorted(cleaned_data.keys()):
            ...
            for i, data in enumerate(class_data):
                ...
                if self.use_augmentation:
                    # 클래스별 특화 증강
                    augmented_data = self._class_specific_augmentation(data, class_name)
                    for aug_data in augmented_data:
                        self.data.append(aug_data)
                        self.labels.append(class_idx)
    
    def _class_specific_augmentation(self, data, class_name):
        """클래스별 특화 증강"""
        augmented = []
        
        # 문제 클래스별 특화 증강
        if class_name in ['ㄹ', 'ㅁ', 'ㅂ', 'ㅅ', 'ㅠ']:  # 낮은 성능 클래스
            # 1. Orientation 센서 강화 증강
            for noise_level in [0.1, 0.15, 0.2]:
                aug_data = data.copy()
                # Orientation 센서에만 강한 노이즈 추가
                aug_data[:, 5:] += np.random.normal(0, noise_level, (20, 3))
            ...
            # 3. 센서 마스킹 (일부 센서 제거)
            for mask_count in [1, 2]:
                aug_data = data.copy()
                mask_indices = np.random.choice(8, mask_count, replace=False)
                aug_data[:, mask_indices] = 0
                augmented.append(aug_data)
        ...
        elif class_name in ['ㅕ', 'ㅠ', 'ㅡ']:  # Flex2 변동성 과다 클래스
            # Flex2 센서 안정화
            for scale in [0.7, 0.8, 0.9, 1.1, 1.2, 1.3]:
                aug_data = data.copy()
                aug_data[:, 1] = aug_data[:, 1] * scale  # Flex2
                augmented.append(aug_data)
        
        elif class_name in ['ㄷ', 'ㅎ']:  # Flex4 변동성 과다 클래스
            # Flex4 센서 안정화
            for scale in [0.7, 0.8, 0.9, 1.1, 1.2, 1.3]:
                aug_data = data.copy()
                aug_data[:, 3] = aug_data[:, 3] * scale  # Flex4
                augmented.append(aug_data)
...
def train_model(model, train_loader, val_loader, epochs=100, device='cuda', class_weights=None):
    """모델 훈련 (클래스별 가중치 적용)"""
    model.to(device)
    ...
    if class_weights is not None:
        ...
        print(f'📊 클래스별 가중치 적용: {class_weights.cpu().numpy()}')
    else:
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), 'enhanced_mlp_model.pth')
        else:
            patience_counter += 1
...
def evaluate_model(model, test_loader, device='cuda'):
    ...
    plt.title('혼동 행렬 (2순위 정제 + 클래스별 가중치)')
    plt.xlabel('예측')
    plt.ylabel('실제')
...
if __name__ == "__main__":
    ...
    # 2. 2순위 정제 (클래스별 특화 증강)
    print('\n🔧 2순위 정제 (클래스별 특화 증강) 중...')
    dataset = EnhancedH5Dataset(cleaned_data, use_augmentation=True)
    print(f'📈 증강 후 총 샘플 수: {len(dataset)}')
    ...
    train_losses, val_losses, val_accuracies = train_model(
        model, train_loader, val_loader, epochs=100, device=device, class_weights=class_weights
    )
    
    # 7. 모델 평가
    model.load_state_dict(torch.load('enhanced_mlp_model.pth'))
    ...
    plt.tight_layout()
    plt.savefig('enhanced_training_process.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f'\n🎉 2순위 정제 및 개선된 훈련 완료!')
    print(f'💾 저장된 파일:')
    print(f'  - enhanced_mlp_model.pth: 최고 성능 모델')
    print(f'  - enhanced_confusion_matrix.png: 혼동 행렬')
    print(f'  - enhanced_training_process.png: 훈련 과정')
