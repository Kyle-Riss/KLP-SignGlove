...
# H5 데이터셋 클래스 (개선됨)
class ImprovedH5Dataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, sequence_length=20, use_augmentation=True):
                                    ...
                                    with h5py.File(file_path, 'r') as f:
                                        ...
                                        if len(sensor_data) >= sequence_length:
                                            ...
                                            if self.use_augmentation:
                                                # 노이즈 추가
                                                noise_data = data + np.random.normal(0, 0.01, data.shape)
                                                self.data.append(noise_data)
                                                self.labels.append(class_idx)
                                                
                                                # 시간 이동 (약간의 변형)
                                                if len(sensor_data) >= sequence_length + 5:
                                                    shifted_data = sensor_data[-(sequence_length+5):-5]
        ...
        print(f"📊 데이터셋 로드 완료: {len(self.data)} 샘플, {len(self.class_names)} 클래스")
...
print('🔧 개선된 간단한 MLP 모델 훈련 시작')

# 데이터셋 로드 (증강 포함)
dataset = ImprovedH5Dataset(
...
train_loader = DataLoader(torch.utils.data.Subset(dataset, train_indices), 
                         batch_size=16, shuffle=True)  # 배치 크기 증가
...
# 개선된 모델 생성
model = ImprovedSimpleMLP(input_size=160, hidden_sizes=[64, 32, 16], num_classes=24, dropout=0.6)
...
print(f'🚀 개선된 MLP 모델 훈련 시작 (디바이스: {device})')
...
for epoch in range(100):
    ...
    if val_loss < best_val_loss:
        ...
        patience = 0
        # 모델 저장
        torch.save(model.state_dict(), 'improved_simple_mlp_model.pth')
    else:
        patience += 1
...
# 최종 테스트
model.load_state_dict(torch.load('improved_simple_mlp_model.pth'))
model.eval()
test_correct = 0
test_total = 0
all_predictions = []
