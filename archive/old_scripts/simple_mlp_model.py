...
# 매우 간단한 MLP 모델 (파라미터 대폭 축소)
class SimpleMLPModel(nn.Module):
    def __init__(self, input_size=2400, hidden_sizes=[32, 16], num_classes=24, dropout=0.7):
...
train_loader = DataLoader(torch.utils.data.Subset(dataset, train_indices), 
                         batch_size=8, shuffle=True)  # 작은 배치 크기
...
# 매우 간단한 모델 생성 (20 * 8 = 160 → 2400으로 확장)
model = SimpleMLPModel(input_size=160, hidden_sizes=[32, 16], num_classes=24, dropout=0.7)
...
optimizer = torch.optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-3)  # 강한 정규화
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=5)

print(f'🚀 간단한 MLP 모델 훈련 시작 (디바이스: {device})')
